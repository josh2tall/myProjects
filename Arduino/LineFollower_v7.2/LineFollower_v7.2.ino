...
void loop()
{
  ...
  else
  {
    ...
    switch(counter)
    {
      ...
      //Into Blue
      case 41:
      turnRight90(2, 0,true,true,'h', false);
      ML = false;
      break;
...
void IRSensorCounter(int sensorSensitivity)
{
  ...
  else if(MLsensorValue[0] >= sensorSensitivity && timer0 > interval && !right && ML && !arrayStop && !back)
  {
    counter++;
    timer0 = 0;
  }
...
void turnLeft90(int mode, int stoppingDelay, bool resetTimer, bool IncrementCounter, char Stop, bool magnet)
{
  ...
  if(mode == 0)
  {
    ...
    do
    {
        MiddleLeft.readCalibrated(MLsensorValue);
    }while(MLsensorValue[0] < 400);
...
void turnRight90(int mode, int stoppingDelay, bool resetTimer, bool IncrementCounter, char Stop, bool magnet)
{
  ...
  else if(mode == 3)
  {
    ...
    //keep turning left until left IR sensor detects two black lines
      ...
      qtraBack.readCalibrated(backArraySensorValues);
      while(backArraySensorValues[4] < 200)
      {
        qtraBack.readCalibrated(backArraySensorValues);
      }
    }
...
void ToBlue(){turn(1,LOW, -30);}
void ToGreen(){turn(2,LOW,-30);}
void ToRed(){turn(3,LOW,-30);}
void ToCyan(){turn(4,LOW,-30);}
void ToSensor(){turn(1,LOW, 30);}
...
//   positioning carousel
//   resetCarousel(LOW);
void ToMagenta(){turn(4,HIGH,50);}
void ToYellow(){turn(3,HIGH,50);}
void ToGrey(){turn(2,HIGH,50);}
