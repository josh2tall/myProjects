...
//Left IR Sensor
QTRSensorsRC Left((unsigned char[]) {
  51
}, NUM_SENSORS_D, TIMEOUT, QTR_NO_EMITTER_PIN);
unsigned int LsensorValue[NUM_SENSORS_D];

//Right IR Sensor
QTRSensorsRC Right((unsigned char[]) {
  50
}, NUM_SENSORS_D, TIMEOUT, QTR_NO_EMITTER_PIN);
unsigned int RsensorValue[NUM_SENSORS_D];

//Middle left IR Sensor
QTRSensorsRC side((unsigned char[]) {
  48
}, NUM_SENSORS_D, TIMEOUT, QTR_NO_EMITTER_PIN);
unsigned int SsensorValue[NUM_SENSORS_D];

//Front Sensor Array
QTRSensorsAnalog qtraFront((unsigned char[]) {
  0, 1, 2, 3, 4, 5, 6, 7
},
...
//Back Sensor Array
QTRSensorsRC qtraBack((unsigned char[]) {
...
int servoUp = 157;
int servoDown = 2;
int retrieveHeight = 130;
int travelHeight = 173;
int retrieveGreyHeight = 145;
...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(interruptButton), STOP, RISING);
  //lcd
...
void turnRight90(int mode, int stoppingDelay, bool resetTimer, bool IncrementCounter, char Stop, bool magnet)
{
  ...
  else if (mode == 3)
  {
    ...
    //keep turning right until right IR sensor detects two black lines
    Right.readCalibrated(RsensorValue);
    while (RsensorValue[0] < 400) {
      Right.readCalibrated(RsensorValue);
    }
...
void ToSensor() {
  turn(1, HIGH, sensorOffset);
}
void SBlue() {
  turn(1, HIGH, blueOffset);
}
void SGreen() {
  turn(2, HIGH, greenOffset);
}
void SRed() {
  turn(3, HIGH, redOffset);
}
void SCyan() {
  turn(4, HIGH, cyanOffset);
}

void SGrey() {
  turn(2, LOW, greyOffset);
}
void SYellow() {
  turn(3, LOW, yellowOffset);
}
void SMagenta() {
  turn(4, LOW, magentaOffset);
}

//  positioning carousel
void ToBlue() {
  turn(2, HIGH, blueOffset);
}
void ToGreen() {
  turn(3, HIGH, greenOffset);
}
void ToRed() {
  turn(4, HIGH, redOffset);
}
void ToCyan() {
  turn(5, HIGH, cyanOffset);
}

void ToGrey() {
  turn(1, LOW, greyOffset);
}
void ToYellow() {
  turn(2, LOW, yellowOffset);
}
void ToMagenta() {
  turn(3, LOW, magentaOffset);
}
...
void magnetDown()
{
  ...
  for (int i = retrieveHeight; i > servoDown; i--) {
    myservo.write(i);
    delay(3);
  }
  delay(1000);
...
void retrieveToken() {
  ...
  switch (color)
  {
    case 'b':
      SBlue();
      dropToken();
      if (roundCounter == 1 || roundCounter == 2 || roundCounter == 3)
        delay(500);
      resetCarouselCCW();
      blueCounter++;
      break;
...
void printColor(double r, double g, double b, double c)
{
  ...
  else if (r > 2500 && r < 6500 && g > 3500 && g < 7500 && b > 4000 && b < 8000)
  {
